...
export function usePasswordStrength(
  ...
  const state = useMemo(() => {
    ...
    const isExactCommon = value.length > 0 && (value.length <= 10 ? COMMON.test(value) : false)
    const isPattern = value.length > 0 && (RUN.test(value) || RUN_UP.test(value))
    const guessable = isExactCommon || isPattern

    let score = 0
    if (value.length > 0) {
      if (isExactCommon && passed < 3) {
        score = 1
      } else if (isPattern && passed > 1) {
        // Apply a mild penalty (-1 level) for sequential patterns, but respect passed rules
        score = Math.max(1, passed - 1)
      } else {
        score = Math.min(rules.length, Math.max(1, passed))
      }
    }
