...
function repMatchLabel(count: number): {
  label: string;
        ...
        <>
          ...
          <div className="hidden md:block">
            ...
            <p className="mt-4 text-xs font-mono font-bold text-[var(--color-fg-muted)] text-center">
              ...
              {filteredClaims.length !== 1 ? "s" : ""} awaiting community
              verification
            </p>
          </div>
          ...
          <div className="md:hidden space-y-3">
            ...
            <p className="text-xs font-mono font-bold text-[var(--color-fg-muted)] text-center pt-2">
              Showing {filteredClaims.length} claim
              {filteredClaims.length !== 1 ? "s" : ""} awaiting verification
            </p>
          </div>
        </>
      )}
  ...
  return (
    <div className="inline-flex items-center gap-2">
      <div
        ...
        role="img"
        aria-label={`${count} of ${max} verifiers`}
      >
        {Array.from({ length: max }).map((_, i) => (
          <span
            ...
            className={cn(
              ...
              i < count &&
                "bg-[var(--color-brand)] border border-[var(--color-brand)] shadow-[0_0_6px_var(--color-brand)]",
              i === count &&
                close &&
                "border-2 border-dashed border-[var(--color-brand)] animate-pulse",
              i === count &&
                !close &&
      ...
      <span
        className={cn(
          ...
          close ? "text-[var(--color-brand)]" : "text-[var(--color-fg-muted)]",
        )}
      >
        {count}/{max}
      </span>
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        deadline.urgent
          ? "bg-[var(--color-v-mislead-bg)]/20 hover:bg-[var(--color-v-mislead-bg)]/40"
    ...
    >
      ...
      <div className="text-center">
        <span className="text-xs font-bold text-[var(--color-fg-2)] capitalize">
          {claim.category}
        </span>
      </div>
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        deadline.urgent &&
          "border-[var(--color-v-mislead-border)] bg-[var(--color-v-mislead-bg)]/10",
      )}
    >
      <button
        type="button"
      ...
      >
        ...
        {claim.imageUrl && (
          <div className="mb-3 rounded-[var(--radius-md)] overflow-hidden border border-[var(--color-border-soft)] max-h-36 bg-[var(--color-surface-2)]">
            <img
              src={claim.imageUrl}
